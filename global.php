<?php
	/* Handles global $db, Common Requests */
    ...
    */
	if(isset($_GET['spellview'])){
		...
		if($_GET['spellview'] > 0){
			$QueryResult = mysqli_query($db, "SELECT * FROM spells_new WHERE `id` = ". $_GET['spellview'] . ";");
	...
	*/
	if(isset($_GET['item_view'])){
		...
		// echo '<style></style>';
		$QueryResult = mysqli_query($db, "SELECT * FROM items WHERE `id` = ". $_GET['item_view'] . ";");
		while($row = mysqli_fetch_array($QueryResult)){	
			// $Content .= BuildSpellInfo($row, 1);
	...
	*/
	if(isset($_GET['spell_view'])){
		...
		require_once('includes/alla_functions.php');
		require_once('modules/ItemEditor/constants_ie.php');
		$QueryResult = mysqli_query($db, "SELECT * FROM spells_new WHERE `id` = ". $_GET['spell_view'] . ";");
		while($row = mysqli_fetch_array($QueryResult)){	
	...
	*/
	if(isset($_GET['spell_view_data_quick'])){
		...
		$result = mysqli_query($db, "SELECT * FROM spells_new WHERE `id` = ". $_GET['spell_view_data_quick'] . ";");
