<?php
	...
	if($_COOKIE['dblogin2']){ 
		$db2 = mysqli_connect($_COOKIE['dbip2'], $_COOKIE['dbuser2'], $_COOKIE['dbpass2'], true) or die("Impossible to connect to " . $_COOKIE['dbip2'] . " : " . mysqli_error());
		mysqli_select_db($_COOKIE['dbname2'], $db2) or die("Impossible to select " . $_COOKIE['dbname2'] . " : " . mysqli_error());
	}
	...
	if($_GET['ZoneID'] && $_GET['Selector'] == 1){
		...
		$QueryResult = mysqli_query($db, $Query, $db); $zone_info = array();
		while($row = mysqli_fetch_array($QueryResult)){ $zone_info = $row; }
		...
		$QueryResult = mysqli_query($db, $Query, $db);
	...
	if($_GET['ZoneID'] && $_GET['CopyTool'] == "deletezone"){
		$Query = "SELECT * FROM `zone` WHERE `zoneidnumber` = ". $_GET['ZoneID'] . " LIMIT 1";
		$QueryResult = mysqli_query($db, $Query, $db);
		while($row = mysqli_fetch_array($QueryResult)){ $ZoneSN = $row['short_name']; }
	...
	if($_GET['ZoneID'] && isset($_GET['VersionToDelete']) && $_GET['Submit'] == 1){
		if($_GET['DeleteType'] == "partial"){
			$Query = "SELECT * FROM `zone` WHERE `zoneidnumber` = ". $_GET['ZoneID'] . " LIMIT 1";
			$QueryResult = mysqli_query($db, $Query, $db);
			while($row = mysqli_fetch_array($QueryResult)){ $ZoneSN = $row['short_name']; $ZoneLN = $row['long_name']; }
			mysqli_query($db, "DELETE FROM `spawn2` WHERE `zone` = '". $ZoneSN . "' AND `version` = '". $_GET['VersionToDelete'] . "';", $db);
		...
		if($_GET['DeleteType'] == "full"){
			...
			# mysqli_query($db, "DELETE FROM grid WHERE id='999999999999' (". $QueryAdd1 . ") AND ;", $db);
			# mysqli_query($db, "DELETE FROM grid_entries WHERE gridid='999999999999' (". $QueryAdd2 . ");", $db);
			...
			$QueryResult = mysqli_query($db, $Query, $db);
			while($row = mysqli_fetch_array($QueryResult)){
				mysqli_query($db, "DELETE FROM spawnentry WHERE spawngroupID='" . $row['spawngroupID'] . "';", $db);
				mysqli_query($db, "DELETE FROM spawngroup WHERE id='" . $row['spawngroupID'] . "';", $db);
				mysqli_query($db, "DELETE FROM npc_types WHERE id='" . $row['npcID'] . "' AND id > 999;", $db);
			...
			mysqli_query($db, "DELETE FROM spawn2 WHERE zone='" . $ZoneSN . "' AND  spawn2.version='" . $_GET['VersionToDelete'] . "';", $db);
			$query_total_output .= "DELETE FROM spawn2 WHERE zone='" . $ZoneSN . "' AND  spawn2.version='" . $_GET['VersionToDelete'] . "';" . "\n"; 
		}
		if($_GET['ObjectsDelete'] == 1){ 
			mysqli_query($db, "DELETE FROM object WHERE zoneid = " . $_GET['ZoneID'] . " AND version = ". $_GET['VersionToDelete'] . ";", $db);
			$query_total_output .= "DELETE FROM object WHERE zoneid = " . $_GET['ZoneID'] . " AND version = ". $_GET['VersionToDelete'] . ";" . "\n"; 			
		}
		if($_GET['DoorsDelete'] == 1){ 
			mysqli_query($db, "DELETE FROM doors WHERE zone = '" . $ZoneSN . "' AND version = " . $_GET['VersionToDelete'] . ";", $db); 
	...
	if($_GET['ZoneID'] && $_GET['CopyToolSubmit']){
		...
		$result = mysqli_query($db, 'SELECT MAX(id) + 1 AS id FROM `spawn2`', $db);
		...
		if($_GET['NPCDATA'] == "existing"){
			...
			if($_GET['NPCGRIDS']){
				/* Grids */
				$result = mysqli_query($db, 'SELECT MAX(id) + 1 AS id FROM `grid` WHERE `zoneid` = "'. $_GET['ZoneID'] . '"', $db);
				$row = mysqli_fetch_assoc($result);
				$ID = $row['id'];
			...
			/* npc_types */
			$result = mysqli_query($db, 'SELECT MAX(id) + 1 AS id FROM `npc_types`', $db);
			$row = mysqli_fetch_assoc($result);
			$ID = $row['id'];
			$Query = "SELECT
				npc_types.*
			...
			/* spawngroup */
			$result = mysqli_query($db, 'SELECT MAX(id) + 1 AS id FROM `spawngroup`', $db);
			$row = mysqli_fetch_assoc($result);
			$ID = $row['id'];
			$Query = "SELECT
				spawngroup.*
	...
	if($_GET['ZoneID'] && $_GET['ImportSelector'] == 1){ 
		...
		$Query = "SELECT * FROM `zone` WHERE `zoneidnumber` = ". $_GET['ZoneID'] . " LIMIT 1";
		$QueryResult = mysqli_query($db, $Query, $db2);
		while($row = mysqli_fetch_array($QueryResult)){
			$ZoneSN = $row['short_name'];
	...
	if($_GET['ZoneID'] && $_GET['ImportTool'] && isset($_GET['Source']) && isset($_GET['Dest'])){
		...
		if($_GET['Doors'] == 1){
			...
			/* Doors */
			$result = mysqli_query($db, 'SELECT MAX(id) + 1 AS id FROM `doors`', $db);
			$row = mysqli_fetch_assoc($result);
			$ID = $row['id'];
			$result = mysqli_query($db, 'SELECT MAX(doorid) + 1 AS id FROM `doors` WHERE `zone` = "' . $ZoneSN .'" AND (version = ' . $_GET['Source'] . ' or version = -1);', $db);
			$row = mysqli_fetch_assoc($result);
			$DoorID = $row['doorid'];
			...
			while ($row = mysqli_fetch_array($QueryResult)) { 
				...
				foreach ($row as $k=>$v){
					if($n > 0 && !is_numeric($k)){
						$type  = mysqli_field_type($QueryResult, $f);
						$name  = mysqli_field_name($QueryResult, $f);
		...
		if($_GET['Objects'] == 1){
			...
			/* Objects */
			$result = mysqli_query($db, 'SELECT MAX(id) + 1 AS id FROM `object`', $db);
			$row = mysqli_fetch_assoc($result);
			$ID = $row['id'];
		...
		while ($row = mysqli_fetch_array($QueryResult)) { 
			...
			$ID++;
			$Query = substr($InsertQuery, 0, -2) . ') VALUES ('. substr($InsertQuery2, 0, -2) . ');';
			mysqli_query($db, $Query, $db); # if(mysqli_error()){ echo '<b style="color:red">' . mysqli_error() . '</b><br>'; }
			$query_total_output .= $Query . "\n"; 
			#echo $Query . '<br>';
		}
